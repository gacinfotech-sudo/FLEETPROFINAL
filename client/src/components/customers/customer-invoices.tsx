  ...
  return (
    <div className="bg-white border rounded-lg p-6 space-y-5 text-sm" id={`invoice-${invoice._id}`}>
      <div className="flex justify-between gap-4">
        ...
        <div className="text-right"><p className="font-bold text-lg capitalize">{invoice.documentType.replace(/_/g, ' ')}</p><p>{invoice.invoiceNumber}</p><p>{new Date(invoice.invoiceDate).toLocaleDateString('en-IN')}</p><Badge className="mt-1 capitalize" variant={invoice.status === 'finalized' ? 'outline' : 'secondary'}>{invoice.status}</Badge></div>
      </div>
      <div className="grid grid-cols-2 gap-4 border-y py-4">
        ...
        <div><p className="text-gray-500">Booking</p><p className="font-semibold">{invoice.bookingSnapshot?.bookingNumber}</p><p>{invoice.bookingSnapshot?.pickupLocation} → {invoice.bookingSnapshot?.dropoffLocation || '-'}</p><p>{invoice.bookingSnapshot?.vehicle?.name} {invoice.bookingSnapshot?.vehicle?.registration}</p><p>{invoice.bookingSnapshot?.pickupDate ? new Date(invoice.bookingSnapshot.pickupDate).toLocaleDateString('en-IN') : ''}</p></div>
      </div>
...
export default function CustomerInvoices({ customerId, customer, bookings, requestedBookingId, onRequestHandled }: {
  ...
  const downloadPdf = async (invoice: any) => {
    ...
    pdf.setFontSize(10);
    const lines = [
      `Document: ${invoice.invoiceNumber}`, `Date: ${new Date(invoice.invoiceDate).toLocaleDateString('en-IN')}`,
    ...
    pdf.save(`${invoice.invoiceNumber.replace(/[^a-z0-9]+/gi, '-')}.pdf`);
  };
  const printInvoice = (invoice: any) => {
    const win = window.open('', '_blank', 'noopener,noreferrer,width=800,height=900'); if (!win) return;
    win.document.write(`<!doctype html><html><head><title>${safe(invoice.invoiceNumber)}</title><style>body{font-family:Arial;padding:40px}.row{display:flex;justify-content:space-between;border-bottom:1px solid #ddd;padding:8px 0}.total{font-size:22px;font-weight:bold}</style></head><body><h1>${safe(invoice.businessSnapshot?.businessName)}</h1><h2>${safe(invoice.documentType.replace(/_/g, ' '))}</h2><div class="row"><span>Document</span><b>${safe(invoice.invoiceNumber)}</b></div><div class="row"><span>Bill to</span><b>${safe(invoice.billingSnapshot?.billingName)}</b></div><div class="row"><span>Booking</span><b>${safe(invoice.bookingSnapshot?.bookingNumber)}</b></div><div class="row total"><span>Total</span><b>${safe(money(invoice.totalAmount))}</b></div><div class="row"><span>Received</span><b>${safe(money(settledReceived(invoice)))}</b></div><div class="row"><span>Balance</span><b>${safe(money(settledBalance(invoice)))}</b></div><script>window.print();window.close();</script></body></html>`); win.document.close();
  };
