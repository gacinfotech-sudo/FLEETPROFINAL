...
// as this task's other test files.
//
// P0-1 FIX (FLEETPRO-PRIORITIZED-FIX-QUEUE.md): this file's `dropDatabase()`
// calls previously ran against whatever `MONGODB_URI` was already set to —
// `process.env.MONGODB_URI || '<isolated name>'` only falls back when the
// var is unset, but nearly every worktree's `.env` sets it to the SHARED
// `fleetpro` dev database. That combination is confirmed to have caused a
// real near-total wipe of the shared database (5x `dropDatabase()`, ~50
// collections each time — see FLEETPRO-CANONICAL-EVIDENCE-REGISTER.md's
// incident report) when this file was run in a normal `.env`-loaded shell.
// Fixed by always using a dedicated, uniquely-named throwaway database,
// unconditionally overriding any inherited MONGODB_URI — this can never
// accidentally resolve to a shared database regardless of environment.
import assert from 'node:assert/strict';
import mongoose from 'mongoose';
import { randomUUID } from 'node:crypto';

process.env.MONGODB_URI = `mongodb://127.0.0.1:27017/fleetpro_test_sales_config_${randomUUID().slice(0, 8)}`;
