...
export class MongoDBStorage implements IStorage {
  ...
  async getAvailableVehicles(tenantId: string, pickupDate: string, returnDate: string, pickupTime?: string, returnTime?: string): Promise<IVehicle[]> {
    try {
      ...
      const objectId = mongoose.Types.ObjectId.isValid(tenantId) ? new mongoose.Types.ObjectId(tenantId) : tenantId;
      const candidates = await Vehicle.find({ tenantId: objectId, status: 'available' }).sort({ createdAt: -1 });
  ...
  async getBooking(id: string, tenantId?: string): Promise<IBooking | undefined> {
    try {
      ...
      if (!tenantId) {
        return await Booking.findOne({ _id: bookingObjectId })
          .populate('vehicleId')
          .populate('driverId')
  ...
  async updateBooking(id: string, data: any, tenantId?: string): Promise<IBooking | undefined> {
    try {
      ...
      if (!tenantId) {
        const result = await collection.findOneAndUpdate({ _id: bookingObjectId }, { $set: data }, { returnDocument: 'after' });
      ...
      let result = await collection.findOneAndUpdate(
        { _id: bookingObjectId, tenantId: tenantObjectId },
        { $set: data },
  ...
  async deleteBooking(id: string, tenantId?: string): Promise<void> {
    try {
      ...
      if (!tenantId) {
        await collection.deleteOne({ _id: bookingObjectId });
        return;
      }
  ...
  async getTenantStats(tenantId: string): Promise<{
    ...
    try {
      ...
      const [revenueResult, totalBookings, fleetSize, activeDrivers] = await Promise.all([
        ...
        db.collection('bookings').countDocuments({ tenantId: objectId }),
        db.collection('vehicles').countDocuments({ tenantId: objectId, status: 'available' }),
        db.collection('drivers').countDocuments({ tenantId: objectId, status: 'available' })
      ]);
  ...
  async findCallSessionByProviderCallId(tenantId: string, providerCallId: string): Promise<ICallSession | null> {
    try {
      const objectId = mongoose.Types.ObjectId.isValid(tenantId) ? new mongoose.Types.ObjectId(tenantId) : tenantId;
      return await CallSession.findOne({ tenantId: objectId, providerCallId });
    } catch (error) {
  ...
  async updateCallSessionFields(
    ...
    try {
      ...
      return await CallSession.findOneAndUpdate(query, { $set: update }, { new: true });
    } catch (error) {
  ...
  async reassignCallSession(
    ...
    try {
      ...
      const existing = await CallSession.findOne(query);
  ...
  async getTelephonyIdentityForUser(tenantId: string, userId: string): Promise<ITelephonyIdentity | null> {
    try {
      const objectId = mongoose.Types.ObjectId.isValid(tenantId) ? new mongoose.Types.ObjectId(tenantId) : tenantId;
      return await TelephonyIdentity.findOne({ tenantId: objectId, userId: userId.toLowerCase() });
    } catch (error) {
  ...
  async getTelephonyIdentitiesForTenant(tenantId: string): Promise<ITelephonyIdentity[]> {
    try {
      const objectId = mongoose.Types.ObjectId.isValid(tenantId) ? new mongoose.Types.ObjectId(tenantId) : tenantId;
      return await TelephonyIdentity.find({ tenantId: objectId }).sort({ userId: 1 });
    } catch (error) {
  ...
  async getBookingsByTenantPaginated(
    ...
    const [rows, total] = await Promise.all([
      collection
        .find({ tenantId: objectId })
        .sort({ createdAt: -1 })
        .skip(skip)
        .limit(limit)
        .toArray() as unknown as Promise<IBooking[]>,
      collection.countDocuments({ tenantId: objectId }),
    ]);
    return { rows, total };
  }
  ...
  async getSubscriptionByTenant(tenantId: string): Promise<ISubscription | undefined> {
    try {
      const objectId = mongoose.Types.ObjectId.isValid(tenantId) ? new mongoose.Types.ObjectId(tenantId) : tenantId;
      return await Subscription.findOne({ tenantId: objectId, status: { $ne: 'CANCELLED' } })
        .populate('planId')
        .sort({ createdAt: -1 });
    } catch {
      return undefined;
  ...
  async getInvoicesByTenant(tenantId: string, limit = 50, offset = 0): Promise<IInvoice[]> {
    const objectId = mongoose.Types.ObjectId.isValid(tenantId) ? new mongoose.Types.ObjectId(tenantId) : tenantId;
    return Invoice.find({ tenantId: objectId })
      .sort({ invoiceDate: -1 })
      .limit(limit)
      .skip(offset);
  }
  ...
  async getPaymentsByTenant(tenantId: string): Promise<any[]> {
    const objectId = mongoose.Types.ObjectId.isValid(tenantId) ? new mongoose.Types.ObjectId(tenantId) : tenantId;
    return PaymentTransaction.find({ tenantId: objectId }).sort({ recordedAt: -1 });
  }
  ...
  async getBillingLedger(tenantId: string): Promise<any[]> {
    const objectId = mongoose.Types.ObjectId.isValid(tenantId) ? new mongoose.Types.ObjectId(tenantId) : tenantId;
    const invoices = await Invoice.find({ tenantId: objectId }).sort({ invoiceDate: -1 });
    const payments = await PaymentTransaction.find({ tenantId: objectId }).sort({ recordedAt: -1 });
