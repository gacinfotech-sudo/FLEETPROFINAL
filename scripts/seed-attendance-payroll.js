...
const {
  ...
  MonthlyPayroll,
  Tenant
} = require('/Users/pradeep/fleetpro-customer360/server/models/index');
...
async function verifySalaryMasters(drivers) {
...
async function main() {
  ...
  try {
    ...
    const { drivers, totalRecords, count: verifiedCount } = await seedAttendance();

    // Verify salary masters
    const masters = await verifySalaryMasters(drivers);
